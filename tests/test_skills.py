...
def test_claude_agent_injects_skills(tmp_path, monkeypatch):
    """ClaudeAgent.build_command should include --append-system-prompt when skills are present."""
    from coding_agents.agents.claude import ClaudeAgent
    from coding_agents.models import ExecutionConfig
    
    # Create a project-local skill (use VALID_SKILL_MD which has name: test-skill)
    skills_dir = tmp_path / "project" / ".coding-agents" / "skills" / "test-skill"
    ...
    # When skills exist, --append-system-prompt is in the command
    assert "--append-system-prompt" in cmd
    idx = cmd.index("--append-system-prompt")
    preamble = cmd[idx + 1]
    assert "test-skill" in preamble
    assert "# Available Skills" in preamble


def test_claude_agent_no_skills(tmp_path, monkeypatch):
    """ClaudeAgent.build_command should NOT include --append-system-prompt when no skills."""
    from coding_agents.agents.claude import ClaudeAgent
    from coding_agents.models import ExecutionConfig
